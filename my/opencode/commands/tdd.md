...
此命令调用 **dev-tdd-guide-ts** 或 **dev-tdd-guide-py** agent 来强制执行测试驱动开发方法论。
...
dev-tdd-guide-ts / dev-tdd-guide-py agent 将：
...
此命令调用位于以下位置的 `dev-tdd-guide-ts` / `dev-tdd-guide-py` agent：
`~/.claude/agents/dev-tdd-guide-ts.md`
`~/.claude/agents/dev-tdd-guide-py.md`

并且可以引用位于以下位置的 `dev-tdd-workflow` skill：
`~/.claude/skills/dev-tdd-workflow/`
