...
此命令调用 **dev-planner** agent 在编写任何代码之前创建全面的实施计划。
...
**关键**：dev-planner agent 在你明确用"是"、"继续"或类似的肯定回答确认计划之前，**不会**编写任何代码。
...
此命令调用位于 `~/.claude/agents/dev-planner.md` 的 `dev-planner` agent
