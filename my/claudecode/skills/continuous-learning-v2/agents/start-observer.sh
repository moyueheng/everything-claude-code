#!/bin/bash
# Continuous Learning v2 - Observer Agent 启动器
#
# 启动后台 observer agent，分析观察记录
# 并创建 instincts。使用 Haiku 模型以节省成本。
#
# 用法：
#   start-observer.sh        # 后台启动 observer
#   start-observer.sh stop   # 停止运行的 observer
#   start-observer.sh status # 检查 observer 是否运行
