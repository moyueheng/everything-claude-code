...
2. **调查：**
   - 什么失败了？导入错误？属性错误？
   - 是动态导入吗？（importlib、getattr 等）
   - 是只在字符串类型注解中引用的吗？
   - 检测工具遗漏了什么？

3. **向前修复：**
   - 在 DELETION_LOG.md 中标记为"不要移除"
   - 如果是动态导入，添加 noqa 注释
   - 更新检测配置（如 vulture 的 whitelist）
...
```python
# 这些导入可能是为了注册插件
import mypackage.plugins.email  # noqa: F401 - 注册 email 插件
import mypackage.plugins.sms     # noqa: F401 - 注册 sms 插件
```
